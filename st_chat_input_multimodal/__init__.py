...
def multimodal_chat_input(
    ...
    # Track "previous value" to achieve single-shot behavior
    if key is None:
        key = "multimodal_chat_input_default"
    
    # Keys for internal bookkeeping
    last_value_key = f"_last_multimodal_value_{key}"
    has_submitted_key = f"_multimodal_has_submitted_{key}"

    # Before the first submission, render the component in the normal layout
    # (centered by the frontend styles). After the first submission, pin it
    # to the bottom using st._bottom, similar to ChatGPT / st.chat_input.
    has_submitted = st.session_state.get(has_submitted_key, False)
    ...
    if has_submitted:
        ...
        # First-time (or pre-first-message) render: no st._bottom, so the
        # component appears in the normal page flow, centered by CSS.
        component_value = _component_func(
            placeholder=placeholder,
            max_chars=max_chars,
            disabled=disabled,
    ...
    if component_value is not None:
        ...
        if component_value != last_value:
            ...
            # Mark this component as having successfully submitted at least once,
            # so subsequent renders will pin it to the bottom.
            st.session_state[has_submitted_key] = True
