...
function MultimodalChatInput({ 
  ...
  const [textAreaHeight, setTextAreaHeight] = useState<number>(is_compact ? 46 : 80) // レイアウトに応じたデフォルトの高さ
  ...
  const isGenerating = Boolean(is_generating)
  const isCompact = Boolean(is_compact)
  ...
  // Placeholder typewriter animation when input is empty and not focused
  useEffect(() => {
    // If there is content or the user is interacting, show full placeholder immediately
  ...
  }, [placeholder, hasContent, isFocused])
  ...
  const handleSubmit = useCallback(() => {
    const generating = Boolean(is_generating)

    // When generating, treat button as a stop button
    if (generating) {
      if (disabled) return
    ...
    // 高さを確実にリセット（レイアウトに応じた最小高さに戻す）
    setTextAreaHeight(isCompact ? 46 : 80)
    setLastFrameHeight(0) // 前回の高さ記録もリセット
    
    // フレーム高さを基本の高さに戻す
    setTimeout(() => {
      const baseHeight = 40
      const minFrameHeight = baseHeight + (isCompact ? 46 : 80) // 基本高さ + テキストエリア最小高さ
      Streamlit.setFrameHeight(minFrameHeight)
    }, 150)
  }, [inputText, uploadedFiles, voiceHook.audioMetadata, disabled, voiceHook.isRecording, voiceHook.isTranscribing, clearFiles, voiceHook.clearAudioMetadata, is_generating])
  ...
  const getPlaceholder = (): string => {
    ...
    return animatedPlaceholder || placeholder
  }

  const canSend = !isGenerating && !disabled && !voiceHook.isRecording && !voiceHook.isTranscribing && hasContent
  const canStop = isGenerating && !disabled
    ...
    <div style={styles.outerContainer}>
      ...
      >
        ...
        <TextInput
          ...
          disabled={disabled || voiceHook.isRecording || voiceHook.isTranscribing || isGenerating}
          maxChars={max_chars}
          style={styles.textArea}
          onHeightChange={handleTextAreaHeightChange}
          minHeight={isCompact ? 46 : 80}
        />
        
        {/* Send button */}
        <button
          onClick={handleSubmit}
          disabled={isGenerating ? !canStop : !canSend}
          style={styles.sendButton}
          title={isGenerating ? "Stop" : "Send (Enter)"}
        >
          {isGenerating ? (
            <svg
              width="18"
